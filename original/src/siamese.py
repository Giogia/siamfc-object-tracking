...
def _create_siamese(net_path, net_x, net_z):
    ...
    for i in range(_num_layers):
        ...
        net_z = set_convolutional(net_z, conv_w, np.swapaxes(conv_b, 0, 1), _conv_stride[i], bn_beta, bn_gamma,
        ...
        if _pool_stride[i] > 0:
            print('\t\tMAX-POOL: size {} and stride {}'.format(_pool_sz, _pool_stride[i]))
