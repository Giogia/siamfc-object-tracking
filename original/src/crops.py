...
def _resize_images(x, size, resample):
    ...
    for i in range(x.shape[0]):
        y.append(_resize_image(x[i], size, resample))
    y = np.stack(y, axis=0)
    return y
"""
"""
...
def extract_crops_core(im, npad, pos_x, pos_y, sz_src):
    c = sz_src / 2
...
def extract_crops_z(im, npad, pos_x, pos_y, sz_src, sz_dst):
    crop = extract_crops_core(im, npad, pos_x, pos_y, sz_src)
...
def extract_crops_x(im, npad, pos_x, pos_y, sz_src0, sz_src1, sz_src2, sz_dst):
    search_area = extract_crops_core(im, npad, pos_x, pos_y, sz_src2)
