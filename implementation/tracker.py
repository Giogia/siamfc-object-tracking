...
def tracker(frame_name_list, b_box_x, b_box_y, b_box_width, b_box_height, final_score_size, filename, image,
    ...
    scale_factors = parameters.hyperparameters.scale_step ** \
                    np.linspace(-np.ceil(parameters.hyperparameters.scale_num / 2), np.ceil(parameters.hyperparameters.scale_num / 2),
    ...
    penalty = np.matmul(hann_window, hann_window)
    ...
    with tf.Session() as sess:
        ...
        for i in range(1, num_frames):
            ...
            # Update scaled sizes
            window_size_x = (1 - parameters.hyperparameters.scale_lr) * window_size_x + parameters.hyperparameters.scale_lr * scaled_window_size_x[best_scale]
            b_box_width = (1 - parameters.hyperparameters.scale_lr) * b_box_width + parameters.hyperparameters.scale_lr * scaled_b_box_width[
                best_scale]
            b_box_height = (1 - parameters.hyperparameters.scale_lr) * b_box_height + parameters.hyperparameters.scale_lr * scaled_b_box_height[
                best_scale]
            ...
            best_score = (1 - parameters.hyperparameters.window_influence) * best_score + parameters.hyperparameters.window_influence * penalty
            ...
            window_size_z = (1 - parameters.hyperparameters.scale_lr) * window_size_z + parameters.hyperparameters.scale_lr * scaled_window_size_z[best_scale]
